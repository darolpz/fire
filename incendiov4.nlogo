...
to setup
  ...
  set initial-trees count patches with [pcolor = 55]
...
to go
  ...
  if all? patches [pcolor != red and pcolor != orange] [user-message "El incendio se ha extinguido" stop ]
...
to tomar-temperatura
  let temperatura random 300
  if temperatura > random 1000 [
    set pcolor orange - 10 ]
end
to check-death
  let temperature random 5
  if temperature > random 1000 [
    set pcolor red - 13 ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
410
Porcentaje quemado
((count patches with [shade-of? pcolor grey]) / count patches) * 100
17
1
11
